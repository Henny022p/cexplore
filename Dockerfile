...
RUN apt-get update \
    && DEBIAN_FRONTEND=noninteractive apt-get -y install apt-transport-https wget dirmngr build-essential git binutils-arm-none-eabi libsndfile1-dev libpng-dev python3 python3-pip && curl -sL https://deb.nodesource.com/setup_12.x | bash - && apt-get -y install nodejs \
...
RUN pip3 install -r /frontends/requirements.txt
EXPOSE 10240
